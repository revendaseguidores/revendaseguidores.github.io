...
function loadTicketsFromServer() {
    const sessionToken = localStorage.getItem('sessionToken');
    fetch(`https://script.google.com/macros/s/AKfycbwg7C0z6SZIEkLTGoVM8VVTdvA7JhxyHW-1rjq-098oSGrNaeDn4tT_o8GV9XGL4VUL/exec?action=get_user_tickets&token=${encodeURIComponent(sessionToken)}`)
...
function viewTicketDetails(ticketId) {
    const sessionToken = localStorage.getItem('sessionToken');
    showLoadingModal();
    fetch(`https://script.google.com/macros/s/AKfycbwg7C0z6SZIEkLTGoVM8VVTdvA7JhxyHW-1rjq-098oSGrNaeDn4tT_o8GV9XGL4VUL/exec?action=get_ticket_details&token=${encodeURIComponent(sessionToken)}&ticketId=${ticketId}`)
    .then(response => response.json())
    .then(data => {
        hideLoadingModal();
        if (data.success) {
